<template>
    <div>
        ...
        <el-dialog title="分公司评级查询：分页选择下载" :visible.sync="downloadBlack" width="30%" v-dialogDrag>
        ...
        <el-dialog title="修改级别" :visible.sync="updFormDialog" width="35%" v-dialogDrag >
            <el-form ref="updForm" :model="updForm" :rules="rules" :label-position="'right'" label-width="100px"  style="margin-left:13%;">
                <el-form-item label="分公司:" prop="branchcompany">
                    <el-input  disabled style="width: 74%;" clearable ref="usercode" type="text" v-model="updForm.branchcompany" ></el-input>
                </el-form-item>
...
export default {
  ...
  data() {
    return {
      titDatas: BRANCHCOM_RATING_TABLE_HEAD,
      tableData: [],
      searchForm: {
        createTimeBegin: '',
        createTimeEnd: '',
  ...
  created() {
    ...
    this.downdetail = idList.indexOf(538) === -1 ? false : true
  },
  watch: {
    downloadBlack() {
  ...
  methods: {
    ...
    searchData() {
      this.$axios
        .post(
          '/BranchCompanyRate/getBranchCompanyRateList',
          qs.stringify({
    ...
    initTimeSet() {
      ...
      this.searchForm.createTimeBegin =
        y + '-' + m.substring(m.length - 2, m.length)
    ...
    getQueryEnum(param) {
      ...
      this.$axios
        .post('/BranchCompanyRate/getRateType', qs.stringify({}))
        .then(res => {
    ...
    downloadBlackData() {
      ...
      if (
        ((this.endPage * 10 - this.startPage * 10 + 1) / 10) * this.pageSize >
        100000
      ) {
      ...
      sendData.ratingresults =
        this.searchForm.ratingresults === '全部'
          ...
          : this.searchForm.ratingresults
      this.$axios
        .post('/BranchCompanyRate/checkDownloadParam', qs.stringify(sendData))
        .then(res => {
          if (res.data.code * 1 === 200) {
            ...
            let url = '/BranchCompanyRate/downloadList'
            if (this.isDetail) {
    ...
    getDetail(item) {
      ...
      this.updForm.branchcompany = item.branchcompany
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.$axios
            .post(
              '/BranchCompanyRate/UpdateBranchCompanyRate',
              qs.stringify({
                Id: this.updForm.id,
