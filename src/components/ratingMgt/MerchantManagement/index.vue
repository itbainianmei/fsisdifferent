    ...
    <div>
        ...
        <el-dialog title="商户评级查询：分页选择下载" :visible.sync="downloadListDialog" width="30%" v-dialogDrag>
            <div style="text-align: center; margin-bottom:20px;">选择下载从
                ...
                <input type="number" min="0" :max="this.totalPageList" class="downClass" v-model="endNumList">页的数据
            </div>
...
export default {
    ...
    data() {
        return {
            ...
            rules: {
                ratingResults: [{ required: true, message: "请选择人工评级等级", trigger: "change" }],
                remarks: [{ max: 200, min: 0, message: "最长长度不能超过200位", trigger: "blur" }]
            },
            startNumList: 0,
            endNumList: 0,
            totalPageList: 0,
    ...
    methods: {
        searchData() {
            ...
            this.$axios.post('', qs.stringify(params)).then(res => {
                if (res.data.code == 200) {
                    this.tableData = res.data.data.result;
                    this.page.totalCount = total;
                    this.totalPageList = res.data.data.pages;
                ...
                this.showDownloadListBtn = false;
        ...
        initSetTime() {
            ...
            this.searchForm.startDate = y + '-' + m.substring(m.length-2, m.length) + '-' + d.substring(d.length-2, d.length) + ' ' + '00:00:00';
            this.searchForm.endDate = y + '-' + m.substring(m.length-2, m.length) + '-' + d.substring(d.length-2, d.length) + ' ' + '23:59:59';
        },
        resetForm() {
