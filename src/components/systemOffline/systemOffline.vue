    ...
    methods: {
      ...
      showMenu(event,data){
          ...
          if(data.mechid === 1){
              ...
              if(target.tagName.toLowerCase() == 'div' || target.tagName.toLowerCase() == 'span'){
                ...
                let _this = this
                window.onclick = function(){_this.showMenuDetail = false}
              }
      ...
      addOfflineSubmit(){
        ...
        if(this.formAddOffline.examarr == ''){
          ...
          this.$axios.post('/OrganizationController/addMech',qs.stringify(this.formAddOffline))
              .then( res => {
...
<style scoped>
  ...
  .menu{position:absolute;width:85px;height:auto;background-color:rgb(36, 44, 60) ;color: #fff;z-index: 9}
...
.serchipt::-webkit-input-placeholder{font-size: 13px}
.ipt {
  width: 200px;
  height: 36px;
  margin-right: 40px;
