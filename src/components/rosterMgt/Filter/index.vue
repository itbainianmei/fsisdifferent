<template>
    <div>
      ...
      <div class="button">
          <div class="BotoomBtn leftRadius" @click="add" data-title='添加' v-if='btnPower.addBtn'>
            <div class="btn-icon addIcon" ></div>
          </div>
          <div class="BotoomBtn" @click="del" data-title='删除' v-if='btnPower.deleteBtn'>
            <div class="btn-icon removIcon"></div>
          </div>
          <div class="BotoomBtn improt-btn" data-title='导入' @click="importList" v-if='btnPower.downList' >
            <div class="btn-icon refreshIcon"></div>
          </div>
          <div class="BotoomBtn rightRadius" data-title='下载' @click="exportList" v-if='btnPower.downDetail'>
...
export default {
  ...
  created() {
    ...
    this.btnPower.deleteBtn = idList.indexOf(579) === -1 ? false : true
    this.btnPower.downList = idList.indexOf(580) === -1 ? false : true
    this.btnPower.downDetail = idList.indexOf(581) === -1 ? false : true
  },
  methods: {
    // 获取维度列表
    getEnumList() {
