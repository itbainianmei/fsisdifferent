    ...
    <div>
      ...
      <div class="dataTable clear">
        <el-table :data="tableData" border style="width:100%" tooltip-effect="dark" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="创建时间" prop="createTime" :formatter="dateTimeFormat"></el-table-column>
          <el-table-column label="更新时间" prop="updateTime" :formatter="dateTimeFormat"></el-table-column>
...
export default {
  ...
  props: [],
  components: { searchForm, addDialog },
  computed: {},
  created() {
    this.getEnumList();
    this.initSearch();
  ...
  methods: {
    // 获取维度列表
    getEnumList() {
      this.$axios.post('/SysConfigController/queryEnum', qs.stringify({type: 132}))
      .then(res => {
    ...
    search(params = {}) {
      this.$axios.post('/inspectWhiteName/query', qs.stringify(params))
      .then(res => {
    ...
    del() {
      if (this.multipleSelection.length === 0) {
        this.$alert('请至少选中一条需要处理的数据', '提示', { type: 'warning', confirmButtonText: '确定'});
      } else if (this.multipleSelection.length > 0) {
        this.$confirm('确认将选中的名单值删除？', '提示', {type: 'warning', confirmButtonText: '确定', cancelButtonText: '取消'}).then(() => {
          const idList = this.multipleSelection.map(item => item.id);
          const ids = idList.join(',');
          this.$axios.post('/inspectWhiteName/delete', qs.stringify({ids})).then((res) => {
            const result = res.data;
            if (result.code == 200) {
              this.$message({
                message: '删除成功',
    ...
    addSubmit(form) {
      this.$axios.post('/inspectWhiteName/add', qs.stringify(form)).then(res => {
        const result = res.data;
        if (result.code == 200) {
          this.$message({
            type: 'success',
