        ...
        <el-collapse-transition>
          <div class="serior hideTimeRightIcon" v-show="serchToggle">
              ...
              <div style='display:inline-block' class='divToggle'>
                    ...
                    <el-button type="primary" class="serchbtn" icon="el-icon-refresh" @click="reset" v-show='showResetBtnGray'></el-button>
              </div>
          </div>
        </el-collapse-transition>
    </div>
    <div class="seniorSearch" id='seniorSearchToggleShow' v-if="showSeniorQueryGray">
      ...
      <el-collapse-transition>
        <div class="serior" v-if="!seniorSearchToggle">
          ...
          <div style="display: inline-block;margin-left: 4%">
            ...
            <el-button type="primary" icon="el-icon-refresh" @click="reset" v-if="showResetBtnGray"></el-button>
          </div>
      ...
      <div class="contentIcon">
        <div class="button">
          <div class="leftButton clear">
            <div class="BotoomBtn leftRadius" @click="addbtn" data-title='添加' v-show='showAddBtnGray'>
              <div class="addIcon"></div>
            </div>
            <div class="BotoomBtn" @click="removeData" data-title='删除' v-show='showDelBtnGray'>
              <div class="removIcon"></div>
            </div>
            <div class="BotoomBtn" style="border: none"  @click="importegray=true" data-title='导入' v-show='showImportBtnGray'>
              <div class="refreshIcon"></div>
            </div>
            <div class="BotoomBtn rightRadius" @click="downloadgray = true" data-title='下载' v-show='showDownloadBtnGray'>
...
export default {
  ...
  data() {
    return {
      showSearchGray: false,//搜索权限
      showResetBtnGray: false,//重置权限
      showSeniorQueryGray: false,//高级搜索权限
      showAddBtnGray: false,//增加
      showDelBtnGray: false,//删除
      showImportBtnGray: false,//导入
      showDownloadBtnGray: false,//下载
      busiNoListSearch: [],
      delGrayDialog: false,
      titleData: [
        {
  ...
  methods: {
    ...
    upload() {
      ...
      this.$axios
        .post("/NameListController/importGrayList", formData)
        .then(res => {
          if (res.data.code === 1) {
    ...
    downloadMb() {
      window.location = encodeURI(
        this.uploadBaseUrl + "/NameListController/exportGrayModel"
      );
    },
    ...
    downloadgrayData() {
      ...
      this.$axios
        .get(
          "/NameListController/exportList?sessionId=" +
    ...
    queryAuthList() {
      ...
      this.showSearchGray = idList.indexOf(135) === -1 ? false : true;
      this.showResetBtnGray = idList.indexOf(136) === -1 ? false : true;
      this.showSeniorQueryGray = idList.indexOf(137) === -1 ? false : true;
      this.showAddBtnGray = idList.indexOf(138) === -1 ? false : true;
      this.showDelBtnGray = idList.indexOf(139) === -1 ? false : true;
      this.showImportBtnGray = idList.indexOf(140) === -1 ? false : true;
      this.showDownloadBtnGray = idList.indexOf(141) === -1 ? false : true;
    }
  },
  created () {
    this.queryAuthList();
