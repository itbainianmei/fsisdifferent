...
import {card,phone,idCard} from '../utils'
  export default{
    ...
    watch:{
      ...
      if(!val){
      document.querySelector('#tradingScene').setAttribute("readOnly",'true')
    ...
    methods:{
      ...
      whiteDownLoad(){
        ...
        if(parseInt(this.pagenum) * ((parseInt(this.endpageno) - parseInt(this.startno) + 1)) > 100000){
        ...
        if(this.state === ''){
           window.location=encodeURI(this.uploadBaseUrl+'/WhiteNameListController/exportWhiteList?startDate='+this.beginTime+'&startnum='+this.startno +'&endnum='+ this.endpageno +'&pagenum='+ this.pagenum +'&endDate='+this.endTime+'&payTool='+this.payment+'&type='+this.whiteListType+'&merchentId='+this.merchantCode+'&bankCard='+this.bankCard+'&phoneNo='+this.phone+'&certifyId='+this.IDCard+'&Ip='+this.ip+'&terminalId='+this.tradingScene)
          this.downloadWhite = false
          ...
          if(this.state == '生效'){
            ...
            window.location=encodeURI(this.uploadBaseUrl+'/WhiteNameListController/exportWhiteList?startDate='+this.beginTime+'&startnum='+this.startno +'&endnum='+ this.endpageno +'&pagenum='+ this.pagenum+'&endDate='+this.endTime+'&payTool='+this.payment+'&type='+this.whiteListType+'&status='+this.state+'&merchentId='+this.merchantCode+'&bankCard='+this.bankCard+'&phoneNo='+this.phone+'&certifyId='+this.IDCard+'&Ip='+this.ip+'&terminalId='+this.tradingScene)
          this.downloadWhite = false
        }
      ...
      search(){
       ...
       searchForm.sessionId = localStorage.getItem('SID')
        ...
        this.$axios.post('/WhiteNameListController/query',qs.stringify(searchForm))
        .then(res => {
          // 商户编号
              ...
              if(this.form.bankCard === undefined || this.form.bankCard === ''){
                if(this.form.phone === undefined || this.form.phone === ''){
                  if(this.form.ip === undefined || this.form.ip === ''){
                    if( this.form.tradingScene === undefined || this.form.tradingScene === ''){
      ...
      deltelSaveBtn(){
            this.$axios.post('/WhiteNameListController/delete',qs.stringify({
                "sessionId":localStorage.getItem('SID'),
...
.searchContentLeft{width: 80%;border-right: 1px solid #e0e0e0;padding-left: 15px;display: inline-block;}
  .beginTime,.endTime,.state,.payment,.whiteListType,.merchantCode,.IDCard,.bankCard,.phone,.ip,.tradingScene{
    display: inline-block;margin-bottom: 20px;width:31%;
  }
.ip,.tradingScene{margin-bottom: 0}
 .editInput{width: 50%}
.span{width: 110px;text-align: right;display: inline-block}
  .searchContentRight{float: right; padding-right: 4%;padding-top: 45px}
...
.addIpt{border-radius: 50px;    width: 90%;
  height: 36px;}
.BotoomBtn {
  width: 44px;
  height: 28px;
...
.clear:after {
  ...
  .listAddLeftWhiteForm{width: 48%;border-right: 1px solid #e0e0e0;float: left;}
.listAddRightWhiteForm{width: 48%;float: left;}
  .listAddRightWhiteForm .editInput{width: 100%}
 .clear:after{content: "";display: block;clear: both;}
  .listAddBottomForm{border-top: 1px solid #e0e0e0;clear: both;width: 100%;}
  .listAddBottomForm .beginTime{width: 45%;margin-top: 20px;padding-left: 15px;}
.listAddBottomForm .endTime{width: 46%;margin-top: 20px;padding-left: 15px}
