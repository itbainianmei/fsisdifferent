    ...
    <div id="tradeandfraud" @click="allarea($event)">
        <div  class="searchBasic">
            ...
            fixed
              ...
              <el-table-column
                prop="receiptAmount"
                v-if="tableDataSec.receiptAmount[0]"
                 sortable
                show-overflow-tooltip
                :render-header="companyRenderHeader"
                :formatter="formater5"
...
export default {
   ...
   computed:{
     ...
     maxMerchantNo100:function(){
      return this.form.customerNo.split(',').length < 100
      if(this.form.customerNo.split(',').length > 100){
        return this.form.jjj.split(',').length = 100
      }
     }
   },
  ...
  methods:{
    ...
    downloadList() {//是否下载
        ...
        window.location = this.url+"/report/dayReceipt/download?" + qs.stringify(self.form)
    },
    drawLine(){
...
const option = {
  ...
  tooltip: {
    ...
    legend: {
        y:'30px',
        x:'center',
        data:[]//'收单交易金额1','收单交易金额2','活跃商户数1','活跃商户数2'
    },
    xAxis: [
        {
